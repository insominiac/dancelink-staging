...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Basic validation
    if (!content.heroTitle || !content.storyTitle) {
...
export async function DELETE() {
  try {
    // Reset to default content in database
    await prisma.siteSettings.upsert({
      where: {
        id: CONTENT_KEY
      },
      update: {
        socialMedia: DEFAULT_ABOUT_CONTENT,
        updatedAt: new Date()
      },
      create: {
        id: CONTENT_KEY,
    ...
    return NextResponse.json({ error: 'Failed to reset about content' }, { status: 500 })
  }
}
